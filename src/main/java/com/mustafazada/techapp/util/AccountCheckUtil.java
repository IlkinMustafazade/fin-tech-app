...
import com.mustafazada.techapp.exception.EqualAccountException;
...
public class AccountCheckUtil {
    ...
    public void checkEqualityOfAccounts(Integer debitAccountNo, Integer creditAccountNo) {
        if (debitAccountNo.equals(creditAccountNo)) {
            throw EqualAccountException.builder().responseDTO(CommonResponseDTO.builder().status(Status.builder()
                    ...
                    .message("Debit account: " + debitAccountNo + " and " +
                            " credit account: " + creditAccountNo + " are the same")
                    .build()).build()).build();
        }
    }
}
